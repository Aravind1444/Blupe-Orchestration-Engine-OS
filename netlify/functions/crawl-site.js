...
export async function handler(event, context) {
    ...
    try {
        ...
        const { url, maxPages, apiKey } = body;
        ...
        if (authResult.user && authResult.user.id !== 'service_role') {
            const { data: allowed, error: rateLimitError } = await supabase.rpc('check_user_rate_limit', {
                ...
                p_endpoint: 'crawl-site',
                p_max_requests: 100,
                p_window_minutes: 60
            });
            if (rateLimitError) {
                console.error('[RateLimit] Error checking rate limit in Crawl Site:', rateLimitError);
            } else if (!allowed) {
                return {
                    statusCode: 429,
                    headers: corsHeaders,
        ...
        const billingResult = await enforceBilling(authResult, 'crawl-site', body);
        ...
        if (!url) {
            return {
                statusCode: 400,
                headers: corsHeaders,
                body: JSON.stringify({ error: "Site URL is required." })
            };
        }

        console.log(`[Netlify] Crawl Site: "${url}"`);
        ...
        if (!response.ok) {
            ...
            console.error('[Netlify] Tavily Crawl Error:', errorText);
            return {
                statusCode: response.status,
                headers: corsHeaders,
                body: JSON.stringify({ error: `Tavily Crawl Error: ${errorText}` })
            };
        }
        ...
        console.error("[Netlify] Crawl Site Error:", error);
        return {
            statusCode: 500,
            headers: corsHeaders,
